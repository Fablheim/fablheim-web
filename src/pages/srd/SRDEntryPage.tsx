...
export default function SRDEntryPage({
  ...
  const entry = entryOverride ?? params.entry ?? '';
    ...
    <MarketingPage>
      <MarketingNavbar
        ...
        links={[
          ...
          { label: 'System Browse', to: rulesBrowsePath(rulesPathContext, system) },
        ]}
      />
      ...
      <section className="mkt-section px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <div className="mb-5 flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.16em] text-[color:var(--mkt-muted)]">
            ...
            <button onClick={() => navigate(rulesBrowsePath(rulesPathContext, system))} className="mkt-tab px-2 py-1">{system}</button>
            <ChevronRight className="h-3 w-3" />
            <button onClick={() => navigate(rulesBrowsePath(rulesPathContext, system, category))} className="mkt-tab px-2 py-1">{category}</button>
            <ChevronRight className="h-3 w-3" />
            <span>{entry}</span>
          </div>
          ...
          {(prevNext.prev || prevNext.next) && (
            <div className="mt-6 flex items-center justify-between border-t border-[color:var(--mkt-border)] pt-5">
              {prevNext.prev ? (
                <button
                  onClick={() => navigate(rulesEntryPath(rulesPathContext, system, category, prevNext.prev!))}
              ...
              {prevNext.next ? (
                <button
                  onClick={() => navigate(rulesEntryPath(rulesPathContext, system, category, prevNext.next!))}
                  className="mkt-tab flex items-center gap-2 px-3 py-2"
                >
                  {prevNext.next}
